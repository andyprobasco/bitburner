...
export class Contract {
  constructor(ns, file, host) {
    this.host = host;
    this.file = file;
    this.type = ns.codingcontract.getContractType(file, host);
    this.data = ns.codingcontract.getData(file, host);
    this.description = ns.codingcontract.getDescription(file, host);
  }

  solve(ns) {
    if (SOLUTIONS[this.type]) {
      const reward = ns.codingcontract.attempt(
        SOLUTIONS[this.type](this.data),
        this.file,
        this.host,
        { returnReward: true }
      );
      if (reward) {
        ns.tprint(`Contract Completed: ${reward}`);
      } else {
        ns.tprint(
          `Contract FAILED, incorrect answer given for type ${this.type}`
        );
      }
    } else {
      ns.tprint(`No solution available for contracts of type "${this.type}"`);
    }
  }
}
